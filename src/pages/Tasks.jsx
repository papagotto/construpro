...
const Tasks = () => {
    ...
    const onDragEnd = (result) => {
        ...
        // Si no hay destino (se soltó fuera) o es el mismo lugar, no hacer nada
        if (!destination || 
        ...
        if (sourceCol === destCol) {
            ...
            // Actualizar el status de la tarea físicamente
                                    ...
                                    <Droppable droppableId={column.id}>
                                        {(provided, snapshot) => (
                                            <div 
                                                ...
                                                className={`p-2 pt-0 space-y-2 border-t border-slate-100 dark:border-slate-800/50 min-h-[50px] transition-colors ${snapshot.isDraggingOver ? 'bg-slate-50 dark:bg-slate-800/20' : ''}`}
                                            >
                                                    ...
                                                    <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
                                                        {(provided, snapshot) => (
                                                            <div
                                                                ...
                                                                className={`bg-white dark:bg-slate-800/50 p-3 rounded-lg border flex flex-col gap-2 transition-shadow ${snapshot.isDragging ? 'shadow-lg border-primary/50' : 'border-slate-100 dark:border-slate-700/50'}`}
                                ...
                                <Droppable droppableId={column.id}>
                                    {(provided, snapshot) => (
                                        <div 
                                            {...provided.droppableProps}
                                            ref={provided.innerRef}
                                            className={`flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar transition-colors ${snapshot.isDraggingOver ? 'bg-blue-50/20 dark:bg-blue-900/10' : ''}`}
                                        >
                                            {tasks[column.id].map((task, index) => (
                                                <Draggable key={task.id} draggableId={task.id.toString()} index={index}>
                                                    {(provided, snapshot) => (
                                                        <div
                                                            ...
                                                            {...provided.draggableProps}
                                                            {...provided.dragHandleProps}
                                                            className={`bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border hover:shadow-md hover:border-primary/30 transition-all cursor-pointer group ${task.completed ? 'opacity-60 grayscale-[0.5]' : ''} ${snapshot.isDragging ? 'shadow-xl border-primary ring-2 ring-primary/10 rotate-2' : 'border-slate-200 dark:border-slate-700'}`}
                                                            onClick={() => navigate(`/tareas/${task.id}`)}
                                                        >
