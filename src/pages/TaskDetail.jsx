...
const TaskDetail = () => {
    ...
    const calculateTimeEstimates = () => {
        if (!task?.recetas_apu || !task?.cantidad_medida) return null;
        
        const totalHorasHombre = Number(task.cantidad_medida) * Number(task.recetas_apu.rendimiento_mano_obra);
        ...
        return {
            totalHorasHombre,
            diasEstimados,
            cumpleMinimo: personasAsignadas >= (task.recetas_apu.personal_minimo || 1)
        };
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <span className={`px-4 py-2 text-xs font-bold rounded-xl border ${
                        task.prioridad === 'urgente' ? 'bg-red-50 border-red-100 text-red-600' : 'bg-slate-50 border-slate-100 text-slate-600'
                    }`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select 
                                    ...
                                    value={task.receta_id || ''}
                                    onChange={(e) => handleUpdateTask({ receta_id: e.target.value })}
                                >
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Cantidad a Ejecutar ({task.recetas_apu?.unidad_medida || 'u'})</label>
                                <input 
                                    type="number"
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-bold text-slate-700 dark:text-white outline-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-primary/20"
                                    value={task.cantidad_medida || ''}
                                    placeholder="0.00"
                                    onChange={(e) => handleUpdateTask({ cantidad_medida: e.target.value })}
                                />
                            </div>
                        </div>
                                    ...
                                    <div className="flex flex-col justify-center">
                                        {!timeEstimates.cumpleMinimo && (
                                            <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-500">
                                                ...
                                                <p className="text-xs font-bold">Personal insuficiente: Este rubro requiere mín. {task.recetas_apu.personal_minimo} personas.</p>
                                            </div>
                                        )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {materialesRequeridos.length === 0 ? (
                                    ...
                                    materialesRequeridos.map(m => {
                                        const cantidadTotal = (Number(task.cantidad_medida) || 0) * Number(m.cantidad_por_unidad) * (Number(m.coeficiente_desperdicio) || 1);
                                        return (
                    ...
                    <div className="bg-white dark:bg-surface-dark p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase">Inicio Estimado</p>
                                    <input 
                                        type="date"
                                        className="text-sm font-bold bg-transparent border-none p-0 focus:ring-0 text-slate-700 dark:text-white"
                                        value={task.fecha_inicio_estimada || ''}
                                        onChange={(e) => handleUpdateTask({ fecha_inicio_estimada: e.target.value })}
                                    />
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase">Cierre Estimado</p>
                                    <input 
                                        type="date"
                                        className="text-sm font-bold bg-transparent border-none p-0 focus:ring-0 text-slate-700 dark:text-white"
                                        value={task.fecha_fin_estimada || ''}
                                        onChange={(e) => handleUpdateTask({ fecha_fin_estimada: e.target.value })}
                                    />
                                </div>
                            </div>
                        </div>
